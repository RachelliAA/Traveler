...
export default function TripsPage({}) {
  ...
  const loadMoreTrips = async (nextPage = page) => {
    ...
    setLoading(true);
    try {
      const { trips: newTrips, total: totalTrips } = await fetchTenTrips(
        nextPage,
        5
      );
  ...
  const onTripClick = (trip) => {
    navigate(`/trip/${trip._id}`, {
      state: {
        trip: trip,
        user: user,
        myTrip: myTrips.some((mt) => mt._id === trip._id),
      },
    });
  };
          ...
          <>
            ...
            {tab === 1 && displayedTrips.length === 0 ? (
              <Typography
                ...
                sx={{ mt: 6, mb: 6 }}
              >
                Not signed up to any trips yet
              </Typography>
            ) : (
